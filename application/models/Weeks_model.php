...
class Weeks_model extends CI_Model
{
	...
	/**
	 * Exclui uma semana com o ID fornecido
	 *
	 * @param   int   $week_id   ID da semana para deletar
	 *
	 */
	...
	*/
	...
	{
		// Primeiro, livre-se de todas as datas atuais para esta semana
		$this->db->where('week_id', $week_id);
		$this->db->delete('weekdates');

		// Informações do banco de dados que permanecem as mesmas
		...
		foreach ($dates as $date) {
			...
			// Verifique se esta data já existe
			$query = $this->db->query("SELECT `date` FROM weekdates WHERE `date`='$date'");
			$rows = $query->num_rows();

			if ($rows == 1) {
				// Temos uma linha em que a data é outro week_id, então mude-o:
