	...
	function BookingCell($data, $key, $rooms, $users, $room_id, $url, $booking_date_ymd = '', $holidays = array())
	{
		...
		if (isset($data[$key])) {
			...
			if (
				...
				$cancel_msg = 'Tem certeza que deseja cancelar este agendamento?';
				if ($user_id != $booking->user_id) {
					$cancel_msg = 'Tem certeza que deseja cancelar este agendamento?\n\n(**) Por favor, tome cuidado, não é seu.';
				}
			...
			if ($booking_status->result === TRUE) {
				...
				$cell['class'] = 'free';
				$cell['body'] = '<a href="' . $book_url . '"><img src="' . base_url('assets/images/ui/accept.png') . '" width="16" height="16" alt="Agendar" title="Agendar" hspace="4" align="absmiddle" />Agendar</a>';
				//$cell['body'] = '<a href="'.$book_url.'" style="height:50px;"></a>';
				if ($booking_status->is_admin) {
					$cell['body'] .= '<input type="checkbox" name="recurring[]" value="' . $url . '" />';
				}
			} else {
				$cell['class'] = 'unavailable';
				$cell['body'] = 'Data Inválida';
			}
		}
						...
						foreach ($periods as $period) {
							...
							foreach ($school['days_list'] as $day_num => $day_name) {
								...
								if ($period->{$key} == '1') {
									...
									// Período não reservável neste dia, não mostrar ou permitir qualquer agendamento
									$html .= '<td class="unavailable" align="center">Indisponível</td>';
								}
							}
