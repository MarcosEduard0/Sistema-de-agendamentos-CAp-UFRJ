...
foreach ($all_bookings as $epeat_booking) {
	...
	$status_label = booking_status_label($epeat_booking);
	$status_icon = booking_status_icon($epeat_booking);
	$status = img([
		'src' => base_url('assets/images/ui/' . $status_icon),
		'title' => $status_label,
		'alt' => $status_label,
	]);
	$date = "<strong>" . $epeat_booking->date->format($date_format) . "</strong>";
	$notes = strlen($epeat_booking->notes)
		? '<br>' . html_escape($epeat_booking->notes)
		: '';

	if ($epeat_booking->date == $booking->date) {
...
echo "<h3>Agendamentos recorrentes</h3>";
echo $links_html;
echo $info_html;
