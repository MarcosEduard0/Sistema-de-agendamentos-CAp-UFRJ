...
class Table
{
	...
	public function __construct(Context $context)
	{
		...
		if (!$this->context->exception) {
			...
			switch ($this->context->columns) {
				case 'periods':
					$col_count = count($this->context->periods) + 1;
					break;
				case 'days':
					$col_count = count($this->context->dates) + 1;
					break;
				case 'rooms':
					$col_count = count($this->context->rooms) + 1;
					break;
				default:
					$col_count = 0;
			}
	...
	public function render()
	{
		...
		$table_open = '<table border="0" bordercolor="#ffffff" cellpadding="2" cellspacing="2" class="bookings-grid">';
		$table_close = '</table>';
	...
	private function traslate_week_day_2_Br($day)
	{
	...
	private function render_cell($params)
	{
		...
		switch ($this->context->display_type) {
			case 'day':
				$data['date'] = $this->context->date_info;
				break;
			case 'room':
				$data['room'] = $this->context->room;
				break;
		}
